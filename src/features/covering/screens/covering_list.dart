...
import '../controllers/covering_list_controller.dart';
import '../models/covering.dart';
import 'covering_detail.dart';

// ═════════════════════════════════════════════════════════════
//  COVERING LIST PAGE — dual-mode (warping dept = read-only,
//                                  covering dept = beam-entry enabled)
//
//  Pass `canRecordBeamEntries: dept == 'covering'` so the same page
//  serves both worker audiences without duplicate screens.
// ═════════════════════════════════════════════════════════════
class CoveringListPage extends StatefulWidget {
  final bool canRecordBeamEntries;
  const CoveringListPage({
    super.key,
...
class _CoveringListPageState extends State<CoveringListPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(children: [
        ...
        Expanded(child: _CoveringList(
          c: c,
          canRecordBeamEntries: widget.canRecordBeamEntries,
        )),
      ]),
    );
  }
  ...
  PreferredSizeWidget _appBar() => AppBar(
        ...
        title: Obx(() => Column(
              ...
              children: [
                const Text('Covering Jobs', style: ErpTextStyles.pageTitle),
                Text(
                  '${c.list.length} records  •  ${c.statusFilter.value.toUpperCase()}'
                  '${widget.canRecordBeamEntries ? "  •  ENTRY" : "  •  VIEW"}',
...
class _CoveringList extends StatelessWidget {
  ...
  const _CoveringList({
    required this.c,
    required this.canRecordBeamEntries,
  });
      ...
      return RefreshIndicator(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (ctx, i) {
            ...
            return _CoveringCard(
              item: c.list[i],
              canRecordBeamEntries: canRecordBeamEntries,
            );
          },
        ),
      );
